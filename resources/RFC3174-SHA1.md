...
**a. Hex digit:** An element of the set {0, 1, ... , 9, A, ... , F}. A hex digit is the representation of a 4-bit string.

**Examples:** 
- 7 = 0111
- A = 1010

**b. Word:** A 32-bit string which may be represented as a sequence of 8 hex digits. To convert a word to 8 hex digits each 4-bit string is converted to its hex equivalent as described in (a) above.
...
**c. Integer representation:** An integer between 0 and 2^32 - 1 inclusive may be represented as a word. The least significant four bits of the integer are represented by the right-most hex digit of the word representation.
...
**d. Block:** A 512-bit string. A block (e.g., B) may be represented as a sequence of 16 words.
...
### b. Addition modulo 2^32

The operation `X + Y` is defined as follows: words X and Y represent integers x and y, where 0 <= x < 2^32 and 0 <= y < 2^32. For positive integers n and m, let `n mod m` be the remainder upon dividing n by m. Compute:
...
The number of "0"s will depend on the original length of the message. The last 64 bits of the last 512-bit block are reserved for the length l of the original message.
...
The padded message will contain 16 * n words for some n > 0. The padded message is regarded as a sequence of n blocks M(1), M(2), ... , M(n). Since the blocks are made from the padded message, it follows that the last block contains the original message length.
